// 路径安全：设备名过滤 + 子路径穿越校验。
// 对应原 Python 版 server._safe_subpath 与上传处的设备名清洗。
package main
...
//
// 与原 Python `"".join(c for c in device if c.isalnum() or c in "-_") or "default"` 等价：
// Python str.isalnum() 对 Unicode 字母数字（含中文）返回 True，
// 这里用 unicode.IsLetter || unicode.IsDigit 匹配该语义。
...
//
// 规则与原 Python _safe_subpath 完全一致：
//  1. name 非空、不以 '.' 开头（隐藏文件）、不含 '/' 或 '\'（禁止路径分隔注入）
//  2. resolve 后必须仍在 SaveDir 全局根之内（防止 base 本身被构造恶意路径后越界）
//
// 注意：第 2 步判断的是 SaveDir 而非 base，与原版一致——
// 原版用 target.relative_to(SAVE_DIR.resolve())，即使 base 是子目录也校验全局根。
...
// 等价于原 Python `Path(f.filename).name`。
